...
\newcommand{\startslide}[3]{%
  ...
  \node[anchor=east,caption] at (13.75,0.24) {Lunabotics \textbullet{} Alestis Puerto Real \textbullet{} HTP A320};
}

\newcommand{\webcamplaceholder}{%
  \begin{scope}
    \fill[white,opacity=0.94] (14.82,0.72) circle (0.38);
    \draw[Teal,line width=0.65pt] (14.82,0.72) circle (0.38);
    \node[font=\fontsize{7.0}{7.2}\selectfont\bfseries,text=Teal] at (14.82,0.72) {LB};
  \end{scope}
}

\newcommand{\stopslide}{\webcamplaceholder\end{scope}\end{tikzpicture}}
\newcommand{\stopslideplain}{\end{scope}\end{tikzpicture}}

% Beamer-style speaker notes. They are intentionally hidden in the slide PDF/PPTX,
% but kept next to each slide in the source as the rehearsal script.
