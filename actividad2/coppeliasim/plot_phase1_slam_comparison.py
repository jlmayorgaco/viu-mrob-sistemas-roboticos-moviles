"""Plot and summarize Phase 1 SLAM algorithm benchmark runs."""
...
def write_markdown(all_metrics: dict[str, dict[str, object]]) -> None:
    ...
    lines = [
        ...
        row("Duracion [s]", "duration_s"),
        row("Longitud de ruta [m]", "path_length_m"),
        row("RMSE posicion [m]", "rmse_position_m"),
        row("Error P95 [m]", "p95_position_error_m"),
        row("Error maximo [m]", "max_position_error_m"),
        row("Features/celdas finales", "map_features_final"),
        row("Updates SLAM", "slam_updates"),
        row("Submaps", "cartographer_submaps_final"),
        row("Loop closures", "loop_closures_final"),
        row("Distancia minima obstaculo [m]", "min_obstacle_m"),
        row("Riesgo maximo", "max_obstacle_risk"),
        row("Bateria usada [%]", "battery_used_pct"),
        "",
        "## Lectura tecnica",
        "",
...
def main() -> int:
    ...
    fig.suptitle("Phase 1 Basic - benchmark SLAM LiDAR", fontsize=15, fontweight="bold")
    ...
    ax.set_title("Trayectoria, estimacion y mapa final")
    ax.set_xlabel("x [m]")
    ax.set_ylabel("y [m]")
    ax.axis("equal")
    ...
    ax.set_title("Error de estimacion de posicion")
    ax.set_xlabel("t [s]")
    ax.set_ylabel("m")
    ax.grid(True, alpha=0.25)
    ax.legend(loc="best")
    ...
    ax.set_xlabel("t [s]")
    ax.set_ylabel("features / celdas ocupadas")
    ax.grid(True, alpha=0.25)
    ax.legend(loc="best")

    ax = axes[1, 1]
    ax.axis("off")
    ax.set_title("Metricas comparativas")
    metric_rows = [
        ("RMSE pos [m]", "rmse_position_m"),
        ("P95 pos [m]", "p95_position_error_m"),
        ("Error max [m]", "max_position_error_m"),
        ("Mapa final", "map_features_final"),
        ("Updates", "slam_updates"),
        ("Submaps", "cartographer_submaps_final"),
        ("Loop closures", "loop_closures_final"),
        ("Min obst [m]", "min_obstacle_m"),
        ("Riesgo max", "max_obstacle_risk"),
        ("Bateria [%]", "battery_used_pct"),
    ]
    table_data = [
    ...
    table = ax.table(
        cellText=table_data,
        colLabels=["Metrica"] + [LABELS[name] for name in RUNS],
        loc="center",
        cellLoc="center",
        colLoc="center",
    )
