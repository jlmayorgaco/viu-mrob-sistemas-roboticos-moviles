...
def write_analysis_markdown(analysis: dict[str, object]) -> None:
    ...
    lines = [
        ...
        "## Desempeno numerico",
        "",
        f"- RMSE posicion: {est['rmse_position_m']} m",
        f"- Error medio de posicion: {est['mean_position_error_m']} m",
        f"- Error P95 de posicion: {est['p95_position_error_m']} m",
        f"- Error maximo de posicion: {est['max_position_error_m']} m",
        f"- RMSE orientacion: {est['rmse_theta_rad']} rad",
        ...
        f"- Frecuencia media de actualizacion: {mapping['updates_per_second']} updates/s",
        f"- Actualizaciones por landmark: {mapping['updates_per_landmark']}",
        f"- Duracion por modo de planificador: {planner_durations}",
        f"- Duracion por modo de movimiento: {motion_durations}",
        "",
        "## Seguridad operacional",
        "",
        f"- Distancia minima positiva a obstaculo: {safety['min_obstacle_m']} m",
        f"- Riesgo maximo de obstaculo: {safety['max_obstacle_risk']}",
        f"- Bateria inicial/final: {safety['battery_start_pct']}% -> {safety['battery_final_pct']}%",
...
def plot_estimator_performance(rows: list[dict[str, float | int | str]]) -> None:
    ...
    fig.suptitle("Phase 1 Basic - desempeno del estimador", fontsize=15, fontweight="bold")

    ax = axes[0, 0]
    ax.plot(t, ex, color="#1f5fa8", linewidth=1.4, label="error x")
    ax.plot(t, ey, color="#c9472c", linewidth=1.4, label="error y")
    ax.plot(t, position_error, color="#10a37f", linewidth=1.7, label="error posicion")
    ax.set_title("Error de posicion")
    ax.set_xlabel("t [s]")
    ax.set_ylabel("m")
    ...
    ax.set_title("Distribucion del error")
