"""Build the Bill-follower scene and run the controller comparison checks."""
...
PIONEER_FOLLOW_DISTANCE_M = 0.82
...
def make_bill_visual_target_only(sim) -> None:
    """Bill should be seen by the sensors, not bump the Pioneer off course."""
...
def create_industrial_chair(
    ...
    if load_library_model(
        sim,
        "furniture/chairs/swivel chair.ttm",
        alias,
        origin,
        parent,
        yaw=yaw,
...
def create_wall_industrial_details(sim, parent: int) -> None:
    ...
    create_box(sim, "A2F_North_Wall_Cable_Tray", (6.80, 0.035, 0.045), (-0.20, 5.155, 0.49), conduit, parent, respondable=False, detectable=False)
    create_box(sim, "A2F_South_Wall_Air_Line", (6.10, 0.035, 0.040), (0.35, -5.155, 0.45), conduit, parent, respondable=False, detectable=False)
    create_box(sim, "A2F_East_Wall_Control_Cabinet", (0.050, 0.58, 0.60), (5.155, 1.72, 0.40), panel, parent, respondable=False, detectable=False)
    create_box(sim, "A2F_East_Wall_Control_Cabinet_Door", (0.055, 0.45, 0.45), (5.125, 1.72, 0.40), (0.12, 0.14, 0.16), parent, respondable=False, detectable=False)
    create_box(sim, "A2F_East_Wall_EStop_Button", (0.060, 0.07, 0.07), (5.09, 1.55, 0.46), (0.78, 0.05, 0.04), parent, respondable=False, detectable=False)
    create_box(sim, "A2F_East_Wall_Status_Light", (0.060, 0.06, 0.06), (5.09, 1.88, 0.52), (0.05, 0.72, 0.26), parent, respondable=False, detectable=False)
    create_box(sim, "A2F_North_Wall_Caution_Plate", (0.50, 0.040, 0.22), (-3.42, 5.148, 0.34), warning, parent, respondable=False, detectable=False)
...
def build_room(sim) -> int:
    ...
    for ix in range(16):
        for iy in range(16):
            x = -4.875 + ix * tile_size
            y = -4.875 + iy * tile_size
    ...
    create_box(sim, "A2F_Room_Wall_North", (10.40, 0.06, 0.55), (0.0, 5.20, 0.275), wall_color, room, respondable=False)
    create_box(sim, "A2F_Room_Wall_South", (10.40, 0.06, 0.55), (0.0, -5.20, 0.275), wall_color, room, respondable=False)
    create_box(sim, "A2F_Room_Wall_East", (0.06, 10.40, 0.55), (5.20, 0.0, 0.275), wall_color, room, respondable=False)
    create_box(sim, "A2F_Room_Wall_West", (0.06, 10.40, 0.55), (-5.20, 0.0, 0.275), wall_color, room, respondable=False)

    create_industrial_table(sim, "A2F_Industrial_Work_Table_NorthEast", (3.90, 4.10, 0.0), math.radians(-7), room)
    create_industrial_table(sim, "A2F_Industrial_Inspection_Table_NorthWest", (-3.90, 4.10, 0.0), math.radians(8), room)
    create_industrial_chair(sim, "A2F_Industrial_Operator_Chair", (3.10, 3.90, 0.0), math.radians(94), room)
    create_storage_shelf(sim, "A2F_Tool_Shelf_East_Wall", (4.70, -0.80, 0.0), 0.0, room)
    create_storage_shelf(sim, "A2F_Parts_Shelf_South_Wall", (2.80, -4.70, 0.0), math.radians(90), room)
    create_pallet_stack(sim, "A2F_Industrial_Pallet_Stack_SouthWest", (-3.90, -3.95, 0.0), math.radians(-8), room)
    create_wall_industrial_details(sim, room)
    create_hazard_stripes(sim, "A2F_Hazard_Texture_North_Service_Aisle", (-1.20, 4.82, 0.0), 0.0, room, count=12)
    create_hazard_stripes(sim, "A2F_Hazard_Texture_South_Service_Aisle", (1.30, -4.82, 0.0), 0.0, room, count=10)
    create_hazard_stripes(sim, "A2F_Hazard_Texture_East_Shelf_Clearance", (4.82, -1.60, 0.0), math.radians(90), room, count=8)

    for index, x in enumerate((-3.90, -2.60, -1.30, 0.0, 1.30, 2.60, 3.90), start=1):
        create_box(sim, f"A2F_Floor_Expansion_Joint_X_{index:02d}", (0.018, 10.08, 0.007), (x, 0.0, 0.011), (0.34, 0.35, 0.36), room, respondable=False, detectable=False)
    for index, y in enumerate((-3.90, -2.60, -1.30, 0.0, 1.30, 2.60, 3.90), start=1):
        create_box(sim, f"A2F_Floor_Expansion_Joint_Y_{index:02d}", (10.08, 0.018, 0.007), (0.0, y, 0.012), (0.34, 0.35, 0.36), room, respondable=False, detectable=False)
    ...
    create_box(sim, "A2F_Control_Mode_Light_P", (0.16, 0.055, 0.12), (-4.04, 5.04, 0.55), (0.88, 0.22, 0.14), room, respondable=False, detectable=False)
    create_box(sim, "A2F_Control_Mode_Light_PI", (0.16, 0.055, 0.12), (-3.72, 5.04, 0.55), (0.92, 0.68, 0.18), room, respondable=False, detectable=False)
    create_box(sim, "A2F_Control_Mode_Light_PID", (0.16, 0.055, 0.12), (-3.40, 5.04, 0.55), (0.12, 0.58, 0.28), room, respondable=False, detectable=False)
    create_box(sim, "A2F_Control_Mode_Light_LQR", (0.16, 0.055, 0.12), (-3.08, 5.04, 0.55), (0.18, 0.42, 0.84), room, respondable=False, detectable=False)
    create_box(sim, "A2F_Control_Mode_Light_NMPC", (0.16, 0.055, 0.12), (-2.76, 5.04, 0.55), (0.00, 0.58, 0.62), room, respondable=False, detectable=False)
    create_box(
        sim,
    ...
    create_dummy(sim, "A2F_CSV_Output_Reference_Point", (4.35, -3.35, 0.36), room, size=0.045)
...
def set_initial_layout(sim) -> None:
    ...
    bill_start_x, bill_start_y = WAYPOINTS[0]
    bill_start_yaw = BILL_START_ANGLE_RAD + math.pi / 2.0
    pioneer_start = (
        bill_start_x - PIONEER_FOLLOW_DISTANCE_M * math.cos(bill_start_yaw),
        bill_start_y - PIONEER_FOLLOW_DISTANCE_M * math.sin(bill_start_yaw),
    )

    if pioneer >= 0:
        sim.setObjectAlias(pioneer, "PioneerP3DX")
        sim.setObjectPosition(pioneer, [pioneer_start[0], pioneer_start[1], 0.1388])
        sim.setObjectOrientation(pioneer, [0.0, 0.0, bill_start_yaw])
    if bill >= 0:
        sim.setObjectAlias(bill, "Bill")
        sim.setObjectPosition(bill, [bill_start_x, bill_start_y, 0.0])
        sim.setObjectOrientation(bill, [0.0, 0.0, bill_start_yaw])
    ...
    if camera >= 0:
        sim.setObjectPosition(camera, [6.60, -7.00, 5.20])
        sim.setObjectOrientation(camera, [math.radians(62), 0.0, math.radians(42)])
...
def attach_scripts(sim, room: int) -> None:
    ...
    documentation = """-- VIU SRM Actividad 2.1 - Bill follower
-- Bill walks two circular laps; Pioneer follows with the selected controller.
-- Use followerControlMode = P, PI, PID, LQR or NMPC before starting.
-- CSV logs go to actividad2/coppeliasim/follower_pid_logs.
-- A2F_ objects belong to the follower workspace and can be rebuilt safely.
"""
